...
def getSleepTime() -> float:
    now = datetime.now()
    next_wake_time = now + datetime.timedelta(minutes=6)
    next_wake_time = next_wake_time.replace(minute=(next_wake_time.minute // 6) * 6, second=0, microsecond=0)
    
    logger.debug(f"Next wake time is {next_wake_time}.")
    
    return (next_wake_time - now).total_seconds()

def main(db_file: str, sensors: dict[str, dict[str, int|str]], debug: bool) -> None:
    logging.basicConfig(
    ...
    while True:
        ...
        sensor_values = sensor_connection.read_sensors()
...
if __name__ == "__main__":
    ...
    argparse.add_argument("--debug", action="store_true", help="Enable debug logging")
    args = argparser.parse_args()

    with open(args.config, "r") as config_file:
        config = json.load(config_file)
        main(config["database_name"], config["sensors"])
